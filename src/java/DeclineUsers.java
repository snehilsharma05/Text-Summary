...
public class DeclineUsers extends HttpServlet
{
		...
		con=DriverManager.getConnection("jdbc:mysql://localhost:3306/project","root","root");
                            ...
                            int nore= stmt.executeUpdate("Update users set status='-1' where id='"+uid+"' ");
                             if(nore>0)
                             {
                                 ...
                                 request.setAttribute("status", "User Declined !");
                                 RequestDispatcher rd=request.getRequestDispatcher("pendingUsers.jsp");
                                 rd.forward(request, response);
                                 MailSender ms=new MailSender();
                                 ms.sendMail(email,"Test","Declined");
                             }
                             else
                             {
                             
                                 request.setAttribute("status", "Can't Decline !");
                                 RequestDispatcher rd=request.getRequestDispatcher("pendingUsers.jsp");
                                 rd.forward(request, response);
